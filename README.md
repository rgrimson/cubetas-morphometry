...
the data distributed with it, with the two documented exceptions listed under
...
are imported, never executed, which is why they carry no number.
...
| `11_fig2_schematic.py` | **Figure 2** | Section 3.5 |
...
Figure 1 is not produced here: see *What is not distributed*. Figure 2 is a
schematic, and nothing in it is measured from data.
...
`Figure_A1`, as PNG and PDF. The Figure 6 file carries a basin suffix
(`Figure_6_RRB`) because the script can also draw the MRB panel, which the paper
does not use.
